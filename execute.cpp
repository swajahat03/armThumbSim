...
void execute() {
  ...
  int statOffset, branchOffset, num1, num2, result, BitCount;
  unsigned int bit;
  ...
  switch(itype) {
    ...
    case LD_ST:
      ...
      switch(ldst_ops) {
        case STRI:
          // functionally complete, needs stats - done
    ...
    case ADD_SP:
      ...
      setNegativeZero(SP + (addsp.instr.add.imm*4));
